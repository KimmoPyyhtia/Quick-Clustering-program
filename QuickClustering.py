...
def z_score(data, data_labels):# calculates the zscore of the data, based on the labels of the data columns
...
def find_best_cluster_n(data, data_labels, n_clusters):# find the best k value between 2 and 20
    sil_score_array=[]
...
pcaPlot(pca2D(data_zscore, data[class_label]), class_label) #plot pca scatterplot with actual attributes

#print(pca2D(data_zscore, data[class_label]))
#print(silhouette_score_kMean(data,data_labels,3))

#find the best amount of clusters based on the best silhouette score, returns three best values, uses kNN-method
score, k_val = find_best_cluster_n(data,data_labels,20) 
